#!/bin/bash
#SBATCH --job-name=rmats_rez_vs_peak
#SBATCH --output=rmats_rez_vs_peak.out
#SBATCH --error=rmats_rez_vs_peak.err
...
find "$BAM_RESOLUTION" -name '*Aligned.sortedByCoord.out.bam' | sort | paste -sd, - > "$OUTDIR/b1.txt"
find "$BAM_PEAK" -name '*Aligned.sortedByCoord.out.bam' | sort | paste -sd, - > "$OUTDIR/b2.txt"

# Log preview
echo "🧪 Resolution BAMs:"; cat "$OUTDIR/b1.txt"
